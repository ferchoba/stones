...
(function ($, angular, stones) {
  ...
  /*
   * Returns a function to retrieve a list of entities by page.
   */
  stones.factory('stones.get_entities_by_page', [
    '$http',
    'stones.server_error_handler',
    '$rootScope',
    '$q',
    ...
    function ($http, server_error_handler, scope, $q, $timeout) {
      return function (url, cache) {
        return function (page, page_size, _params) {
          var deferred = $q.defer(),
  ...
  /*
   * Returns a function to push a PUT request on one entity by key or id.
   *
   * url parameter must have a key variable to be replaced by key or id from
   * entity. For example:
   *   /entity_type/{key}/method_we_want_to_put/
   * Note that "key" should be surrounded by "{}".
   */
  stones.factory('stones.put_entity', [
    '$http',
    'stones.server_error_handler',
    '$rootScope',
    function ($http, server_error_handler, scope) {
      return function (url) {
        return function (entity, data) {
          var http_opts = {
            url: url.assign({key: (entity.$$key$$ || entity.$$id$$)}),
            method: 'PUT',
            data: data
          };
  ...
  /*
   * Adds a tooltip for title attribute.
  ...
  /*
   * Adds a popover.
   * Popover is provided by bootstrap.
   */
  stones.directive('stPopover', [
    function(){
      return {
        restrict: 'A',
        link: function(scope, _elm, attrs) {
  ...
  /*
   * Adds html safe escaping.
   */
  stones.filter('stSafe', [
    '$sce',
    function ($sce) {
      return function (input) {
...
(function ($, angular, stones){
  ...
  stones.auth.run([
    ...
    function ($rootScope, $window) {
      $rootScope.logged_user = $window.logger_user;
    }]);
